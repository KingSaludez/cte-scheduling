...
<head>
    ...
    <title>Faculties - {{ config('app.name', 'CTE NEMSU Tagbina') }}</title>
...
<div class="main">
    <div class="page-header"><h1>Faculties <span>· Manage faculties</span></h1><a href="{{ route('faculties.create') }}" class="btn btn-primary">+ Add Faculty</a></div>
    @if(session('success'))<div class="alert alert-success">{{ session('success') }}</div>@endif
    <div class="card"><div class="card-body">
        <form method="GET" class="filter-bar">
            <input type="text" name="search" placeholder="Search by name..." value="{{ request('search') }}">
            <select name="employment_status"><option value="">All Status</option><option value="full-time" {{ request('employment_status') == 'full-time' ? 'selected' : '' }}>Full-time</option><option value="part-time" {{ request('employment_status') == 'part-time' ? 'selected' : '' }}>Part-time</option></select>
            <button type="submit" class="btn btn-primary btn-sm">Filter</button>
        </form>
        <div class="table-wrap"><table><thead><tr><th>Full Name</th><th>Status</th><th>Rank</th><th>Specialization</th><th>User Account</th><th>Actions</th></tr></thead><tbody>@forelse($faculties as $faculty)<tr><td>{{ $faculty->full_name }}</td><td><span class="badge {{ $faculty->employment_status == 'full-time' ? 'badge-green' : 'badge-golden' }}">{{ ucfirst($faculty->employment_status) }}</span></td><td>{{ $faculty->academic_rank ?? 'N/A' }}</td><td>{{ $faculty->specialization ?? 'N/A' }}</td><td>{{ $faculty->user->email ?? 'Not linked' }}</td><td><div class="actions"><a href="{{ route('faculties.show', $faculty) }}" class="btn btn-primary btn-sm">View</a><a href="{{ route('faculties.edit', $faculty) }}" class="btn btn-warning btn-sm">Edit</a><form action="{{ route('faculties.destroy', $faculty) }}" method="POST" onsubmit="return confirm('Delete this faculty?');">@csrf @method('DELETE')<button type="submit" class="btn btn-danger btn-sm">Delete</button></form></div></td></tr>@empty<tr><td colspan="6"><div class="empty">No faculties found.</div></td></tr>@endforelse</tbody></table></div>
